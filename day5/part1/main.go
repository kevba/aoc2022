...
func solve(lines []string) string {
	...
	for _, m := range moves {
		for i := 0; i < m.amount; i++ {
			moveIndex := len(state[m.from]) - 1
			value := state[m.from][moveIndex]
			state[m.from] = append(state[m.from][:moveIndex], state[m.from][moveIndex+1:]...)
			state[m.to] = append(state[m.to], value)
		}
	}
